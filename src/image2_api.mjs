...
export function buildImage2EditApiPayload(prompt, config = {}, inputImages = []) {
  const merged = { ...DEFAULT_IMAGE2_API_CONFIG, ...config };
  const payload = {
    model: merged.model,
    prompt: String(prompt || ""),
    ...
    output_format: merged.output_format,
    moderation: merged.moderation,
    images: inputImages.map(normalizeImageInputReference)
  };
...
export async function generateImage2File(options = {}) {
  ...
  const payload = editMode
    ? buildImage2EditApiPayload(options.prompt, options.config, inputImages)
    : buildImage2ApiPayload(options.prompt, options.config);
  const endpoint = editMode ? "https://api.openai.com/v1/images/edits" : "https://api.openai.com/v1/images/generations";
  const fetchImpl = options.fetchImpl || fetch;
  const maxTransientRetries = Number(options.config?.max_transient_retries ?? DEFAULT_IMAGE2_API_CONFIG.max_transient_retries);
  ...
  return {
    ...
    input_image_count: editMode ? inputImages.length : 0,
...
function normalizeImageInputReference(input) {
  if (typeof input === "string") return { image_url: input };
  if (input?.image_url) return { image_url: input.image_url };
  if (input?.file_id) return { file_id: input.file_id };
  throw new Error("image input must include image_url or file_id.");
}
